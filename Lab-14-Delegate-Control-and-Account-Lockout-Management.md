# Overview: Lab 14 Delegate Control and Account Lockout Management
...
To investigate a user account that has been locked out, we can use a tool from Microsoft called the **Account Lockout and Management Tools**. We can download it from Microsoft’s website. After downloading, we’ll move the file into our SimoTech Lab folder, which is shared with our Windows Server 2022 VM.
...
If we run the application again and search for Andrew, the results will show that the account is no longer locked.
